// App.js
import React, { useState } from 'react';
import { NavigationContainer } from '@react-navigation/native';
import { createNativeStackNavigator } from '@react-navigation/native-stack';
import { createBottomTabNavigator } from '@react-navigation/bottom-tabs';
import { Ionicons } from '@expo/vector-icons';

import Login from './screens/Login';
import SignUp from './screens/SignUp';
import Home from './screens/Home';
import VendorDetails from './screens/VendorDetails';
import Cart from './screens/Cart';
import Orders from './screens/Orders';
import Profile from './screens/Profile';

const Stack = createNativeStackNavigator();
const Tab = createBottomTabNavigator();

export default function App() {
  const [cartItems, setCartItems] = useState([]); // [{id, name, price, vendorName, quantity}]
  const [orders, setOrders] = useState([]);       // [{id, code, items, total, pickupTime, status, vendorName}]

  function addToCart(item, vendorName) {
    setCartItems((prev) => {
      const index = prev.findIndex(
        (c) => c.id === item.id && c.vendorName === vendorName
      );
      if (index !== -1) {
        const updated = [...prev];
        updated[index].quantity += 1;
        return updated;
      }
      return [
        ...prev,
        {
          id: item.id,
          name: item.name,
          price: item.price,
          vendorName,
          quantity: 1,
        },
      ];
    });
  }

  function decreaseFromCart(itemId) {
    setCartItems((prev) =>
      prev
        .map((item) =>
          item.id === itemId
            ? { ...item, quantity: item.quantity - 1 }
            : item
        )
        .filter((item) => item.quantity > 0)
    );
  }

  function clearCart() {
    setCartItems([]);
  }

  function getCartTotal() {
    return cartItems.reduce(
      (sum, item) => sum + item.price * item.quantity,
      0
    );
  }

  function generatePickupCode() {
    const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    const letter = letters[Math.floor(Math.random() * letters.length)];
    const number = Math.floor(10 + Math.random() * 89); // 10–99
    return `${letter}${number}`;
  }

  function placeOrder() {
    if (cartItems.length === 0) return null;

    const total = getCartTotal();
    const code = generatePickupCode();

    const order = {
      id: Date.now().toString(),
      code,
      items: cartItems,
      total,
      vendorName:
        cartItems.length === 1
          ? cartItems[0].vendorName
          : 'Multiple Vendors',
      pickupTime: '10:30 AM',
      status: 'Ready for Pickup',
    };

    setOrders((prev) => [order, ...prev]);
    clearCart();
    return order;
  }

  const sharedProps = {
    cartItems,
    orders,
    addToCart,
    decreaseFromCart,
    placeOrder,
    getCartTotal,
  };
  // ================================================================

  function MainTabs(propsFromStack) {
    // propsFromStack carries navigation props, but also we can use sharedProps directly
    return (
      <Tab.Navigator
        screenOptions={({ route }) => ({
          headerShown: false,
          tabBarActiveTintColor: '#276EF1',
          tabBarInactiveTintColor: '#9BA3B4',
          tabBarLabelStyle: { fontSize: 12 },
          tabBarIcon: ({ color, size }) => {
            let iconName = 'home-outline';
            if (route.name === 'Home') iconName = 'home-outline';
            if (route.name === 'Cart') iconName = 'cart-outline';
            if (route.name === 'Orders') iconName = 'clipboard-outline';
            if (route.name === 'Profile') iconName = 'person-outline';
            return <Ionicons name={iconName} size={size} color={color} />;
          },
        })}
      >
        <Tab.Screen name="Home">
          {(tabProps) => (
            <Home
              {...tabProps}
              // you can pass addToCart if you later want quick add from Home
            />
          )}
        </Tab.Screen>

        <Tab.Screen name="Cart">
          {(tabProps) => (
            <Cart
              {...tabProps}
              cartItems={cartItems}
              decreaseFromCart={decreaseFromCart}
              placeOrder={placeOrder}
              getCartTotal={getCartTotal}
            />
          )}
        </Tab.Screen>

        <Tab.Screen name="Orders">
          {(tabProps) => <Orders {...tabProps} orders={orders} />}
        </Tab.Screen>

        <Tab.Screen name="Profile" component={Profile} />
      </Tab.Navigator>
    );
  }

  return (
    <NavigationContainer>
      <Stack.Navigator initialRouteName="Login">
        <Stack.Screen
          name="Login"
          component={Login}
          options={{ headerShown: false }}
        />
        <Stack.Screen
          name="SignUp"
          component={SignUp}
          options={{ title: 'Create Account' }}
        />
        {/* MainTabs receives shared state through props */}
        <Stack.Screen
          name="MainTabs"
          options={{ headerShown: false }}
        >
          {(stackProps) => <MainTabs {...stackProps} {...sharedProps} />}
        </Stack.Screen>

        {/* VendorDetails also needs addToCart */}
        <Stack.Screen
          name="VendorDetails"
          options={{ title: 'Menu' }}
        >
          {(stackProps) => (
            <VendorDetails {...stackProps} addToCart={addToCart} />
          )}
        </Stack.Screen>
      </Stack.Navigator>
    </NavigationContainer>
  );
}